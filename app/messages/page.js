...
export default function Messages() {
  ...
  useEffect(() => {
    const read = () => setSelectedId(new URLSearchParams(window.location.search).get("user"));
    read();
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("connection_requests").select("id, sender_id, receiver_id, status").or(`sender_id.eq.${user.id},receiver_id.eq.${user.id}`).then(({ data }) => setRequests(data || []));
  }, [user]);
  ...
  useEffect(() => {
    ...
    async function loadChat() {
      ...
      setSelectedProfile(profileData || null);
      setMessages(messageError ? [] : (messageData || []));
      if (profileError || !profileData) setMessage("We couldn't find that profile.");
      else if (messageError) setMessage("We couldn't load this conversation yet. Check the direct_messages policies in Supabase.");
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`dm-${user.id}-${selectedId}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "direct_messages" }, ({ new: row }) => {
      if ((row.sender_id === user.id && row.receiver_id === selectedId) || (row.sender_id === selectedId && row.receiver_id === user.id)) {
  ...
  const requestByPerson = useMemo(() => {
    const map = new Map();
    requests.forEach((r) => map.set(r.sender_id === user?.id ? r.receiver_id : r.sender_id, r));
    return map;
  }, [requests, user]);
  ...
  async function sendRequest(receiverId) {
    ...
    if (error) setMessage(error.code === "23505" ? "A connection request already exists." : "Couldn't send the request yet.");
    else setRequests((prev) => [...prev, { sender_id: user.id, receiver_id: receiverId, status: "pending" }]);
  }
  ...
  async function sendMessage(e) {
    ...
    setMessages((prev) => [...prev, { id: optimisticId, sender_id: user.id, receiver_id: selectedProfile.id, body, created_at: new Date().toISOString() }]);
    setDraft("");
    ...
    if (error) {
      ...
      setDraft(body);
      setMessage("Your message couldn't be sent. Please check the direct_messages policies in Supabase.");
    } else setMessages((prev) => prev.map((m) => m.id === optimisticId ? data : m));
    setSending(false);
  }
