...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      let request = supabase.from("profiles").select("id, full_name, username, avatar_url, avatar_seed").neq("id", user.id).not("full_name", "is", null).order("full_name").limit(50);
      if (term) request = request.or(`full_name.ilike.%${safeTerm}%,username.ilike.%${safeTerm}%`);
      const { data, error } = await request;
      setProfiles(error ? [] : (data || []).filter((p) => p.full_name?.trim() || p.username?.trim()));
  ...
  useEffect(() => {
    ...
    async function loadChat() {
      ...
      const [{ data: profileData, error: profileError }, { data: messageData, error: messageError }] = await Promise.all([
        supabase.from("profiles").select("id, full_name, username, avatar_url, avatar_seed").eq("id", selectedId).maybeSingle(),
  ...
  return (
    <main className="page"><section className="shell">
      <header className="header"><p className="eyebrow">THREDORI CIRCLE</p><h1>Find your people <span>♡</span></h1><p className="subtitle">Search by full name or @username, connect, and start a conversation.</p></header>
