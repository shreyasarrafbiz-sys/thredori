    ...
    <header>
      <div className="topbar">
        ...
        <div className="search-wrap page-rise">
          ...
          {searchValue && <button className="clear-search" type="button" onClick={() => onSearch?.("")} aria-label="Clear search">×</button>}
        </div>
      ...
      <style jsx>{`
        ...
        .search-icon { width:34px; height:34px; flex:0 0 34px; display:inline-flex; align-items:center; justify-content:center; color:var(--ink); border-radius:50%; background:rgba(255,255,255,.6); box-shadow:inset 0 1px 0 rgba(255,255,255,.8); }
        ...
        .clear-search { width:24px; height:24px; flex:0 0 24px; border:0; border-radius:50%; background:rgba(255,255,255,.5); color:var(--muted); font-size:18px; line-height:1; cursor:pointer; }
