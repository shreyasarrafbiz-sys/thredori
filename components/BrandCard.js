  ...
  return (
    <div className="brand-card">
      ...
      {brand.isReal && (
        <div className="action-row">
          ...
          <Link href={`/post/${brand.id}`} className="comment-btn" aria-label={`${commentCount} comments`} title={`${commentCount} comments`}>
            ...
            {commentCount > 0 && <span className="comment-count">{commentCount > 99 ? "99+" : commentCount}</span>}
          </Link>
      ...
      <style jsx>{`
        ...
        .comment-btn { position:relative; width:28px; height:28px; flex:0 0 28px; display:inline-flex; align-items:center; justify-content:center; border:1px solid var(--cotton-line); border-radius:50%; background:#fffdfc; color:var(--ink); font-size:15px; text-decoration:none; transition:transform 180ms ease,box-shadow 180ms ease,color 180ms ease,background 180ms ease; }
        ...
        .comment-count { position:absolute; top:-5px; right:-5px; min-width:13px; height:13px; padding:0 3px; display:inline-flex; align-items:center; justify-content:center; border-radius:999px; background:var(--madder); color:#fff; border:2px solid #fffdfc; font:700 7px/1 var(--font-sans); letter-spacing:-.02em; }
